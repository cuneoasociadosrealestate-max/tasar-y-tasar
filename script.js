...
const FACTORES_TASACION = {
  ...
  piso: {
    pisosBajos: { min: 1, max: 2, factor: 0.92 },
    pisosIdeales: { min: 3, max: 8, factor: 1.0 },
    pisosAltos: { min: 9, max: 15, factor: 0.96 },
    pisosExtremosAltos: { min: 16, max: 999, factor: 0.88 }
  },
  ascensor: {
    sinAscensor: {
      piso1_3: 1.0,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function validarInputs(datos) {
    ...
    if (datos.piso < 1) errores.push("El piso debe ser 1 o mayor");
